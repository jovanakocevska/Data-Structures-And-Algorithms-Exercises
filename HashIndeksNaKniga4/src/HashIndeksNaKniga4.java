...
// CBHT, edna fraza i povekje strani, <String, String>
public class HashIndeksNaKniga4 {
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			fraza = pom[0];
			brStrana = pom[1].trim();
			tmp = tabela.search(fraza);
			if(tmp == null) {
				tabela.insert(fraza, brStrana);
			} else {
				novBrStrana = tmp.element.value + ",";
				novBrStrana += brStrana;
				tabela.insert(fraza, novBrStrana);
			}
			for (i = 0; i < pomZborovi.length; i++) {
				tmp = tabela.search(pomZborovi[i]);
				if(tmp == null) {
					tabela.insert(pomZborovi[i], brStrana);
				} else {
					novBrStrana = tmp.element.value + ",";
					novBrStrana += brStrana;
					tabela.insert(pomZborovi[i], novBrStrana);
				}
			}
		}
