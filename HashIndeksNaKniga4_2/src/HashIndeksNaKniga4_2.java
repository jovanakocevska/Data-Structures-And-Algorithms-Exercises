...
// OBHT, edna fraza i povekje strani, <String, ArrayList<Integer>>
public class HashIndeksNaKniga4_2 {

	public static void main(String[] args) throws IOException {
		// TODO Auto-generated method stub
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		String s, brStrana, fraza, rezultat = "";
		...
		while (true) {
			...
			fraza = pom[0];
			brStrana = pom[1].trim();
			h = tabela.search(fraza);
			if(h == -1) {
				al = new ArrayList<Integer>();
				al.add(Integer.parseInt(brStrana));
				tabela.insert(fraza, al);
			} else {
			...
			for (i = 0; i < pomZborovi.length; i++) {
				h = tabela.search(pomZborovi[i]);
				if(h == -1) {
					al = new ArrayList<Integer>();
					al.add(Integer.parseInt(brStrana));
					tabela.insert(pomZborovi[i], al);
				} else {
					al = tabela.buckets[h].value;
					al.add(Integer.parseInt(brStrana));
					tabela.insert(pomZborovi[i], al);
